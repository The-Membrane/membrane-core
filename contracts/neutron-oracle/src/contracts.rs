...
use membrane::types::{NeutronOracleInfo, Basket, Owner, PoolInfo, PoolStateResponse, PriceInfo, TWAPPoolInfo, VaultTokenInfo};
use membrane::neutron_oracle::{Config, AssetResponse, ExecuteMsg, InstantiateMsg, PriceResponse, QueryMsg, MigrateMsg};
use membrane::neutron_oracle::{query_slinky_price, assert_slinky, };
...
fn edit_asset(
    ...
    if remove && config.positions_contract.is_some() {
        //Check 
        //Query each positions contract for asset being removed
        let basket: Basket = deps.querier.query_wasm_smart(config.positions_contract.unwrap(), &CDP_QueryMsg::GetBasket {  })?;
    ...
    } else if let Some(oracle_info) = oracle_info {
        //Update Asset
        ASSETS.save(deps.storage, asset_info.to_string(), &oracle_info)?;
        ...
        Ok(oracles) => {
                ...
                .into_iter().any(|oracle| oracle.basket_id == oracle_info.basket_id)
            {
                ASSETS.update(
                    ...
                    |oracle| -> Result<Vec<NeutronOracleInfo>, ContractError> {
                        match oracle {
...
pub fn get_vault_token_price(
    ...
    let asset_info = String::NativeToken {
...
pub fn get_lp_price(
    ...
    let asset_infos: Vec<String> = pool_info.clone().asset_infos
        .into_iter()
...
fn get_assets(deps: Deps, asset_infos: Vec<String>) -> StdResult<Vec<AssetResponse>> {
    let mut resp = vec![];
...
fn get_asset_price(
    ...
    let oracle_info = ASSETS.load(storage, asset_info.to_string())?;

    let mut oracle_prices = vec![];

    //Use Slinky USD-quoted price feeds
    if let Some(base_symbol) = oracle_info.clone().slinky_base_symbol {
        if let Some(max_blocks_old) = oracle_info.clone().slinky_max_blocks_old {
            // Assert Slinky configuration
            assert_slinky(&Deps { storage, querier: querier.clone(), api: querier.api() }, &base_symbol, oracle_info.decimals as u8, max_blocks_old)?;
            
            // Get USD price for Slinky scaling (we'll use static USD price for now)
            let usd_price = STATIC_USD_PRICE;
            
            //Query USD price from Slinky
            let slinky_price = query_slinky_price(
                &Deps { storage, querier: querier.clone(), api: querier.api() },
                &env,
                &base_symbol,
                oracle_info.decimals as u8,
                max_blocks_old,
                usd_price,
            )?;

            //Push Slinky USD price
            oracle_prices.push(PriceInfo {
                source: String::from("slinky"),
                price: slinky_price,
            });
            ...
            return Err(StdError::GenericErr {
                msg: String::from("Slinky max_blocks_old not configured"),
            });
        }
    } else {
        return Err(StdError::GenericErr {
            msg: String::from("Slinky base_symbol not configured"),
        });
    }
