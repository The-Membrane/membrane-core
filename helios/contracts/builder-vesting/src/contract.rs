...
use membrane::staking::{ ExecuteMsg as StakingExecuteMsg, QueryMsg as StakingQueryMsg, RewardsResponse };
use membrane::types::{ VestingPeriod, Allocation, Asset, AssetInfo };
use membrane::math::decimal_division;


use crate::error::{ ContractError };
use crate::state::{ CONFIG, Config, RECEIVERS, Receiver };
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::DecreaseAllocation { receiver, allocation } => decrease_allocation(deps, info, receiver, allocation ),
        ...
        ExecuteMsg::SubmitProposal { title, description, link, messages } => submit_proposal(deps, info, title, description, link, messages),
        ExecuteMsg::CastVote { proposal_id, vote } => cast_vote(deps, info, proposal_id, vote),
        ExecuteMsg::UpdateConfig { owner, mbrn_denom, osmosis_proxy, staking_contract } => update_config( deps, info, owner, mbrn_denom, osmosis_proxy, staking_contract ),
    }
}
    ...
    match receivers.clone().into_iter().enumerate().find(|(_i, receiver)| receiver.receiver == info.sender ) {
        Some( (i, receiver) ) => {
            if receiver.claimables == vec![] {
                return Err( ContractError::CustomError{ val: String::from("Nothing to claim") } )
            }
...
fn claim_fees_for_contract(
    ...
    if res.claimables != vec![] {
        ...
        let mut allocated_receivers: Vec<Receiver> = receivers.clone()
            .into_iter()
        ...
        for claim_asset in res.clone().claimables {
            ...
            for ( i, receiver ) in allocated_receivers.clone().into_iter().enumerate() {
                ...
                match receiver.clone().claimables.into_iter().enumerate().find(|(_index, claim)| claim.info == claim_asset.info) {
                    ...
                    None => {
                        allocated_receivers[i].claimables.push(
                            Asset {
        ...
        let mut new_receivers: Vec<Receiver> = receivers.clone()
            .into_iter()
    ...
    let msg = CosmosMsg::Wasm(WasmMsg::Execute {
        ...
        msg: to_binary(
                &StakingExecuteMsg::ClaimRewards { 
                    claim_as_cw20: None,
    ...
    //Claimables into String List
    let claimables_string: Vec<String> = res.claimables
        .into_iter()
        .map(|claim| claim.to_string() )
        .collect::<Vec<String>>();
...
fn get_allocation_ratios(
    ...
    for receiver in receivers.clone() {
        ...
        allocation_ratios.push( 
            decimal_division( 
                Decimal::from_ratio(receiver.clone().allocation.unwrap().amount, Uint128::new(1u128)), 
    ...
    match receivers.clone().into_iter().find(|receiver| receiver.receiver == info.sender ){
        Some( mut receiver ) => {
            if receiver.allocation.is_some(){
                (unlocked_amount, new_allocation) = get_unlocked_amount( receiver.allocation, env.block.time.seconds() );
...
fn add_allocation(
    ...
    if allocation_total > config.initial_allocation { return Err( ContractError::OverAllocated {} ) }
            ...
            .map(|mut current_receiver| {
                ...
                if current_receiver.receiver.to_string() == receiver{
                    ...
                    if current_receiver.allocation.is_some(){
                        ...
                        match current_receiver.allocation.clone().unwrap().amount.checked_sub(allocation){
                            Ok( difference ) => { 
                                current_receiver.allocation = Some( Allocation {
                                    amount: difference,
...
fn add_receiver(
    ...
    RECEIVERS.update( deps.storage, | mut receivers| -> Result<Vec<Receiver>, ContractError> {
        ...
        if receivers.iter().find(|receiver| receiver.receiver == valid_receiver).is_some(){
            return Err( ContractError::CustomError { val: String::from("Duplicate receiver") } )
        }
...
fn query_allocation(
    ...
    let receiver = match RECEIVERS.load( deps.storage )?.into_iter().find(|stored_receiver| stored_receiver.receiver.to_string() == receiver){
        Some( receiver ) => { receiver },
        None => { return Err( StdError::GenericErr { msg: String::from("Invalid receiver") } ) }
    };
    ...
    } else { return Err( StdError::GenericErr { msg: String::from("Receiver has no allocation") } )}
}
...
fn query_receivers(
    deps: Deps,
) -> StdResult<Vec<ReceiverResponse>>{
    let receivers = RECEIVERS.load( deps.storage )?;
...
fn query_receiver(
    ...
    match receivers.into_iter().find(|stored_receiver| stored_receiver.receiver.to_string() == receiver ) {
        Some( stored_receiver ) => {
            Ok(
                ReceiverResponse {
...
pub fn asset_to_coin(
    ...
    match asset.info{
        ...
        AssetInfo::Token { address: _ } => 
            return Err(StdError::GenericErr { msg: String::from("CW20 Assets can't be converted into Coin") })
        ,
...
mod tests {
    ...
    fn receivers() {
        ...
        //AddReceiver
        let add_msg = ExecuteMsg::AddReceiver { receiver: String::from("receiver0000") };
        let _res = execute(deps.as_mut(), mock_env(), mock_info("owner0000", &[]), add_msg).unwrap();

        
        //Error: Duplicate Receiver
        let add_msg = ExecuteMsg::AddReceiver { receiver: String::from("receiver0000") };
        let err = execute(deps.as_mut(), mock_env(), mock_info("owner0000", &[]), add_msg).unwrap_err();
        assert_eq!( err.to_string(), String::from("Custom Error val: \"Duplicate receiver\"") );

        //RemoveReceiver
        let add_msg = ExecuteMsg::RemoveReceiver { receiver: String::from("receiver0000") };
        let _res = execute(deps.as_mut(), mock_env(), mock_info("owner0000", &[]), add_msg).unwrap();
    ...
    fn allocations() {
        ...
        let _res = execute(deps.as_mut(), mock_env(), mock_info("owner0000", &[]), allocation_msg).unwrap();
        ...
        let msg = QueryMsg::Allocation { receiver: String::from("receiver0000") };
        let res = query( deps.as_ref(), mock_env(), msg)
        .unwrap();
        ...
        //AddReceiver
        let add_msg = ExecuteMsg::AddReceiver { receiver: String::from("receiver1") };
        let _res = execute(deps.as_mut(), mock_env(), mock_info("owner0000", &[]), add_msg).unwrap();
        ...
        let err = execute(deps.as_mut(), mock_env(), mock_info("owner0000", &[]), allocation_msg).unwrap_err();
        assert_eq!( err.to_string(), String::from("Increase is over contract's allocation") );
    }
    ...
    fn vesting_unlocks() {
        ...
        let res = execute(deps.as_mut(), env.clone(), mock_info("receiver0000", &[]), withdraw_msg).unwrap();
        ...
        let withdraw_msg = ExecuteMsg::WithdrawUnlocked {  };
        let res = execute(deps.as_mut(), env, mock_info("receiver0000", &[]), withdraw_msg).unwrap();
