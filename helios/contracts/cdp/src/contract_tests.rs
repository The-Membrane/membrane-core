...
mod tests {
    ...
    use membrane::positions::{ExecuteMsg, InstantiateMsg, PositionResponse, QueryMsg, PositionsResponse, BasketResponse, ConfigResponse};
    ...
    fn collateral_supply_caps(){
        ...
        //Instantiating contract
        let v_info = mock_info("owner", &coins(1, "credit"));
        let _res = instantiate(deps.as_mut(), mock_env(), v_info.clone(), msg.clone()).unwrap();
        ...
        let msg = ExecuteMsg::EditBasket { 
            ...
            collateral_supply_caps: Some( vec![ Uint128::new(100u128), Uint128::new(200u128) ]),
        ...
        let info = mock_info("sender88", &coins(101, "debit"));
        let err = execute(deps.as_mut(), mock_env(), info.clone(), exec_msg).unwrap_err();
        assert_eq!(err.to_string(), String::from("Custom Error val: \"Collateral pushes total over basket supply caps: 101 / 100\"")  );
        ...
        let info = mock_info("sender88", &coins(201, "2nddebit"));
        let err = execute(deps.as_mut(), mock_env(), info.clone(), exec_msg).unwrap_err();
        assert_eq!(err.to_string(), String::from("Custom Error val: \"Collateral pushes total over basket supply caps: 201 / 200\"")  );
