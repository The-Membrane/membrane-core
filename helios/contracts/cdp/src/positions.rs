...
use membrane::types::{Asset, Basket, Position, cAsset, AssetInfo, SellWallDistribution, LiqAsset, UserInfo, TWAPPoolInfo, StoredPrice, AssetOracleInfo, SupplyCap, LiquidityInfo};
...
use membrane::stability_pool::{Cw20HookMsg as SP_Cw20HookMsg, QueryMsg as SP_QueryMsg, PoolResponse, ExecuteMsg as SP_ExecuteMsg, DepositResponse};
        ...
        Ok( positions) => {
            ...
            if position_id.is_some(){
                ...
                if position.is_some() {
                    ...
                    accrue( deps.storage, deps.querier, env.clone(), &mut position.clone().unwrap(), &mut basket)?;
                    //Save Basket
                ...
                //Accrue, mainly for repayment price                                    
                accrue( deps.storage, deps.querier, env.clone(), &mut new_position, &mut basket)?;
        ...
        match target_position.clone().collateral_assets.into_iter().find(|x| x.asset.info.equal(&withdraw_asset.info)){
            ...
            Some( position_collateral ) => {                
                ...
                if withdraw_asset.amount > position_collateral.asset.amount{
                    ...
                    if insolvency_check(deps.storage, env.clone(), deps.querier, basket.clone(), updated_cAsset_list.clone(), Decimal::from_ratio(target_position.clone().credit_amount, Uint128::new(1u128)), basket.credit_price, true, config.clone())?.0{ 
...
) ->Result<Response, ContractError>
{
   ...
   //Position repayment
    ...
    let collateral_assets = get_LP_pool_cAssets( deps.querier, config.clone(), basket.clone(), target_position.clone().collateral_assets)?;
    let cAsset_ratios = get_cAsset_ratios(deps.storage, env.clone(), deps.querier, collateral_assets.clone(), config.clone())?;
    let (_avg_borrow_LTV, _avg_max_LTV, _total_value, cAsset_prices) = get_avg_LTV(deps.storage, env.clone(), deps.querier, config.clone(), basket.clone(), collateral_assets.clone())?;
    ...
    for (num, cAsset) in collateral_assets.clone().into_iter().enumerate(){
        ...
        let collateral_w_fee = (decimal_multiplication(collateral_repay_amount, sp_liq_fee) + collateral_repay_amount) * Uint128::new(1u128);
...
pub fn increase_debt(
    ...
    if basket.oracle_set{
        ...
        if insolvency_check( deps.storage, env.clone(), deps.querier, basket.clone(), target_position.clone().collateral_assets, Decimal::from_ratio(total_credit, Uint128::new(1u128)), basket.credit_price, true, config.clone())?.0 { 
            //panic!("{}", );
            return Err(ContractError::PositionInsolvent {  })
        }else{
...
pub fn liquidate(
    ...
    let (_insolvent, _current_LTV, _available_fee) = insolvency_check( storage, env.clone(), querier, basket.clone(), target_position.clone().collateral_assets, Decimal::from_ratio(target_position.clone().credit_amount, Uint128::new(1u128)), basket.credit_price, false, config.clone())?;
    //TODO: Delete
    let insolvent = true;
    ...
    let (avg_borrow_LTV, avg_max_LTV, total_value, cAsset_prices) = get_avg_LTV( storage, env.clone(), querier, config.clone(), basket.clone(), target_position.clone().collateral_assets)?;
    ...
    for ( i, cAsset ) in target_position.clone().collateral_assets.into_iter().enumerate(){
        ...
        if cAsset.clone().pool_info.is_some(){
            ...
            let lp_liquidate_amount = decimal_multiplication(cAsset_ratios[i], decimal_division(avg_borrow_LTV, current_LTV)) * cAsset.asset.amount;
    ...
    let collateral_assets = get_LP_pool_cAssets(querier, config.clone(), basket.clone(), target_position.clone().collateral_assets)?;
    let cAsset_ratios = get_cAsset_ratios( storage, env.clone(), querier, collateral_assets.clone(), config.clone() )?;
   ...
   // panic!("{}", credit_repay_amount);
    ...
    for (num, cAsset) in collateral_assets.clone().iter().enumerate(){
        ...
        let caller_fee_in_collateral_amount = decimal_multiplication(collateral_repay_amount, caller_fee) * Uint128::new(1u128);
        ...
        if basket.clone().liq_queue.is_some(){
            ...
            let res: LQ_LiquidatibleResponse = querier.query(&QueryRequest::Wasm(WasmQuery::Smart {
                ...
                msg: to_binary(
                    &LQ_QueryMsg::CheckLiquidatible {
                        ...
                        collateral_price,
                        collateral_amount: Uint256::from( (collateral_repay_amount * Uint128::new(1u128)).u128() ),
            ...
            let queue_asset_amount_paid: Uint128 = (collateral_repay_amount * Uint128::new(1u128)) - leftover;
    ...
    if !( leftover_repayment ).is_zero() && leftover_position_value <= decimal_multiplication( leftover_repayment, basket.clone().credit_price ) {
        ...
        Ok (       
            res.add_messages( lp_withdraw_messages )
...
pub fn create_basket(
    ...
    for (i, asset) in collateral_types.iter().enumerate(){
        ...
        if asset.pool_info.is_some() {            
            return Err( ContractError::CustomError{ val: String::from("Can't add an LP when creating a basket") } )
        } else {
...
pub fn edit_basket(//Can't edit basket id, current_position_id or credit_asset. Can only add cAssets. Can edit owner. Credit price can only be changed thru the accrue function.
    ...
    if added_cAsset.is_some(){
        ...
        //No duplicates
        if let Some(_duplicate) = basket.clone().collateral_types.into_iter().find(|cAsset| cAsset.asset.info.equal(&new_cAsset.asset.info)){
            ...
            for ( i, asset) in pool_assets.iter().enumerate(){
                ...
                if config.clone().oracle_contract.is_some(){
                    ...
                    deps.querier.query::<AssetResponse>(&QueryRequest::Wasm(
                        WasmQuery::Smart { 
                            contract_addr: config.clone().oracle_contract.unwrap().to_string(), 
                            msg: to_binary( &OracleQueryMsg::Asset { asset_info: AssetInfo::NativeToken { denom: asset.clone().denom } } )? 
                    }))?;
                ...
                if info.sender.clone() != config.owner && info.sender.clone() != basket.owner{
                    ...
                    if collateral_supply_caps.is_some(){
                        ...
                        for new_cap in collateral_supply_caps.unwrap() {
                            if let Some( (index, _cap) ) = basket.clone().collateral_supply_caps.into_iter().enumerate().find(|( _x, cap)| cap.asset_info.equal(&new_cap.asset_info) ) {
...
pub fn get_cAsset_ratios(
    ...
    let (cAsset_values, _cAsset_prices) = get_asset_values(storage, env, querier, collateral_assets.clone(), config, None)?;
...
pub fn insolvency_check( //Returns true if insolvent, current_LTV and available fee to the caller if insolvent
    ...
    let avg_LTVs: (Decimal, Decimal, Decimal, Vec<Decimal>) = get_avg_LTV(storage, env, querier, config, basket, collateral_assets)?;
...
fn update_basket_tally(
    ...
    for cAsset in collateral_assets.iter(){
        ...
        if cAsset.clone().pool_info.is_some(){
            ...
            for ( i, asset ) in pool_info.asset_infos.into_iter().enumerate() {
                ...
                if let Some( (index, mut cap) ) = basket.clone().collateral_supply_caps.into_iter().enumerate().find(|( _x, cap)| cap.asset_info.equal(&asset.info) ) {
            ...
            if let Some( (index, mut cap) ) = basket.clone().collateral_supply_caps.into_iter().enumerate().find(|( _x, cap)| cap.asset_info.equal(&cAsset.asset.info) ) {
    ...
    let mut new_basket_ratios = get_cAsset_ratios(storage, env, querier, temp_cAssets.clone(), config.clone() )?;
                    ...
                    for ( pa_index, pool_asset ) in pool_info.clone().asset_infos.into_iter().enumerate() {
                        ...
                        if let Some(( i, pool_asset_cAsset )) = temp_cAssets.clone().into_iter().enumerate().find(|( _x, asset )| asset.asset.info.equal(&pool_asset.info)) {
                            ...
                            if !pool_asset_cAsset.asset.amount.is_zero(){
                                ...
                                let cap_amount = decimal_multiplication( ratio, new_basket_ratios[i] );
...
fn get_credit_asset_multiplier(
 ...
 fn update_debt_per_asset_in_position(
    ...
    let old_ratios = get_cAsset_ratios(storage, env.clone(), querier, new_old_assets.clone(), config.clone())?;
    ...
    if over_cap {
        return Err( ContractError::CustomError { val: format!("Assets over debt cap: {:?}", assets_over_cap ) } )
    }
...
fn accrue(
    ...
    if !(time_elapsed == 0u64) && basket.oracle_set {
        ...
        let credit_TWAP_price = get_asset_values( storage, env.clone(), querier, vec![ credit_asset ], config.clone(), Some( basket.clone().basket_id ) )?.1[0];
