...
use membrane::oracle::{ ExecuteMsg, InstantiateMsg, QueryMsg, PriceResponse, AssetResponse };
...
use crate::state::{ Config, CONFIG, ASSETS };
...
pub fn execute(
    ...
    match msg {
        ExecuteMsg::AddAsset { asset_info, oracle_info } => add_asset( deps, info, asset_info, oracle_info ),
        ExecuteMsg::EditAsset { asset_info, oracle_info, remove } => edit_asset( deps, info, asset_info, oracle_info, remove ),
        ExecuteMsg::UpdateConfig{ owner, osmosis_proxy, positions_contract } => update_config(deps, env, info, owner, osmosis_proxy, positions_contract),
    }
}
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        ...
        QueryMsg::Price { asset_info, twap_timeframe, basket_id } => to_binary( &get_asset_price( deps.storage, deps.querier, asset_info, twap_timeframe, basket_id )? ),
        QueryMsg::Prices { asset_infos, twap_timeframe } => to_binary( &get_asset_prices(deps, asset_infos, twap_timeframe)? ),
...
fn get_asset_price(
    ...
    let oracle_info = if let Some( oracle_info ) = asset_oracle_info
        ...
        return Err( StdError::GenericErr { msg: String::from("Invalid basket_id") } )
    };
    ...
    if oracle_info.static_price.clone().is_some(){
        ...
        for pool in oracle_info.clone().osmosis_pools_for_twap {
            ...
            let res = querier.query::<ArithmeticTwapToNowResponse>(&QueryRequest::Wasm(WasmQuery::Smart {
    ...
    let decimal_len = Decimal::from_ratio( Uint128::new( oracle_prices.len() as u128), Uint128::new(1u128) );
...
fn get_asset_prices(
    ...
    for asset in asset_infos {
        ...
        price_responses.push( get_asset_price( deps.storage, deps.querier, asset, twap_timeframe.clone(), None )? );
