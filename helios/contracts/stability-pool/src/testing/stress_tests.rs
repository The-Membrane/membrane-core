...
fn simulate_bids_with_2_liq_amounts(
    ...
    assert_eq!(resp.claims[0].to_string(), format!("{} debit", total_distributed));
    assert_eq!(resp.claims[1].to_string(), format!("{} 2nddebit", total_distributed));
    ...
    assert_eq!(total_deposits.to_string(), format!("{}", total_bids-total_liquidated) );
}