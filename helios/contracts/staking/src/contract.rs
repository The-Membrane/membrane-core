...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::UpdateConfig { 
            ...
            fee_wait_period,
            unstaking_period,
         } => update_config(deps, info, owner, positions_contract, builders_contract, osmosis_proxy, mbrn_denom, staking_rate, fee_wait_period, unstaking_period, dex_router, max_spread),
        ...
        ExecuteMsg::DepositFee { } => {
            ...
            if info.clone().sender != config.positions_contract.unwrap(){ return Err( ContractError::Unauthorized {  } ) }
...
pub fn receive_cw20(
    ...
    match from_binary(&cw20_msg.msg){
        Ok( Cw20HookMsg::DepositFee { }) => {
            ...
            if cw20_msg.sender != config.positions_contract.unwrap(){ return Err( ContractError::Unauthorized {  } ) }
...
pub fn stake(
    ...
    if info.funds.len() == 1 && info.funds[0].denom == config.mbrn_denom{
        ...
        return Err( ContractError::CustomError { val: "No valid assets".to_string() } )
    }
    ...
    TOTALS.save(deps.storage, &totals)?;
        ...
        return Err( ContractError::CustomError { val: String::from("Builders contract needs to be added to the config") } )
    }
...
pub fn unstake(
    ...
    let total_stake = {
        ...
        let staker_deposits: Vec<StakeDeposit> = STAKED.load( deps.storage )?
        .into_iter()
        .filter(|deposit| deposit.staker == info.clone().sender)
        .collect::<Vec<StakeDeposit>>();

        if staker_deposits == vec![] {
            return Err( ContractError::CustomError{ val: String::from("User has no stake") } )
        }
    ...
    if withdraw_amount > total_stake {
        return Err( ContractError::CustomError{ val: String::from("Invalid withdrawal amount") } )
    }
    
    //info.sender is user
    let (claimables, accrued_interest, withdrawable_amount) = withdraw_from_state(deps.storage, env, info.clone().sender, withdraw_amount, fee_events)?;
    ...
    if !withdrawable_amount.is_zero(){
        ...
        native_claims.push( asset_to_coin( 
            Asset { 
...
fn withdraw_from_state(
        ...
        .map( |mut deposit| {
            ...
            if deposit.staker == staker{
                ...
                if deposit.unstake_start_time.is_some(){
                    ...
                    if env.block.time.seconds() - deposit.clone().unstake_start_time.unwrap() >= config.unstaking_period {
                        withdrawable = true;
                    }
                }
    ...
    if withdrawal_amount != Uint128::zero(){
        return Err( StdError::GenericErr { msg: format!("Attempting to withdraw {} MBRN over ( {} )'s total deposit", withdrawal_amount, staker) } )
    }
...
pub fn claim_rewards(
    ...
    (messages, user_claimables, accrued_interest) = user_claims( deps.storage, deps.api, env.clone(), config.clone(), info.clone(), config.clone().dex_router, claim_as_native.clone(), claim_as_cw20.clone(), send_to.clone() )?;    
    ...
    if config.osmosis_proxy.is_some(){
        if info.sender != config.clone().builders_contract.unwrap() && !accrued_interest.is_zero(){
            ...
            if send_to.is_some(){
                ...
                let message = CosmosMsg::Wasm(WasmMsg::Execute {
                    ...
                    msg: to_binary(
                            &ExecuteMsg::Stake { user: Some( info.sender.to_string() ) } )?,
                    funds: vec![ coin( accrued_interest.u128(), config.mbrn_denom ) ],
                });
                messages.push( message );

            } else { //Send stake to sender
                let message = CosmosMsg::Wasm(WasmMsg::Execute {
        ...
        return Err(ContractError::CustomError { val: String::from("No proxy contract setup") })
    }
...
fn deposit_fee(
    ...
    if info.sender != config.positions_contract.unwrap() && !cw20_contract { return Err( ContractError::Unauthorized {} ) }
    ...
    if total.is_zero(){ total = Uint128::new(1u128) }
...
fn user_claims(
    ...
    let (user_claimables, accrued_interest) = get_user_claimables(storage, env, info.clone().sender)?;
...
fn get_user_claimables(
    ...
    let deposits: Vec<StakeDeposit> = STAKED.load( storage )?
        .into_iter()
        .filter(|deposit| deposit.staker == staker )
        .collect::<Vec<StakeDeposit>>();

    if deposits == vec![]{
        return Err( StdError::GenericErr{ msg: String::from("User has no stake") } )
    }
    ...
    for deposit in deposits {
        ...
        let (deposit_claimables, deposit_interest) = get_deposit_claimables( config.clone(), env.clone(), fee_events.clone(), deposit.clone())?;
        accrued_interest += deposit_interest;
    ...
    let mut new_deposits: Vec<StakeDeposit> = STAKED.load( storage )?
        .into_iter()
        .filter(|deposit| deposit.staker != staker )
        .collect::<Vec<StakeDeposit>>();
...
fn get_deposit_claimables(
    ...
    for event in events_experienced {       
        ...
        //Check if asset is already in the list of claimables and add according
        match claimables.clone().into_iter().enumerate().find(|(_i, asset)| asset.info == event.fee.info) {
            Some( (index, _asset) ) => { 
                
                claimables[index].amount += event.fee.amount * deposit.amount 
            },
            None => claimables.push( 
                Asset {
                    info: event.fee.info,
...
pub fn query(deps: Deps, env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        ...
        QueryMsg::Staked{ limit, start_after, end_before, unstaking } => to_binary(&query_staked(deps, env, limit, start_after, end_before, unstaking )?),
        QueryMsg::FeeEvents{ limit, start_after } => to_binary(&query_fee_events(deps, limit, start_after )?),
...
fn query_config(
    ...
    Ok(
        ConfigResponse {
            ...
            positions_contract: config.positions_contract.unwrap_or(Addr::unchecked("None")).to_string(),
            builders_contract: config.builders_contract.unwrap_or(Addr::unchecked("None")).to_string(),
            osmosis_proxy: config.osmosis_proxy.unwrap_or(Addr::unchecked("None")).to_string(),
...
fn query_user_stake(
    ...
    let deposit_list = staker_deposits.clone()
        .into_iter()
...
fn query_staked(
    ...
    let mut stakers = STAKED.load( deps.storage )?
        .into_iter()
